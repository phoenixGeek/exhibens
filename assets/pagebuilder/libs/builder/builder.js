				...
				for (i in components)
				{
					...
					if (component)
					{
						...
						if (component.image) {
							...
							item.css({
								backgroundImage: "url(" + '../../assets/pagebuilder/libs/builder/' + component.image + ")",
...
/* drag and drop */
	...
	{
		let data = {};
		data["page_id"] = jQuery("#page-id").val();
		let hashValue = jQuery("#csrf").val();
		let token_name = jQuery("#csrf").attr("name");
		...
		$.ajax({
			type: "POST",
			url: 'admin_ajax/save_page_content',//set your server side save script url
			dataType: 'json',
			data: data,
			cache: false,
			success: function (data) {
				...
				if (data.error) {
					$("#message-modal").modal().find(".modal-body").html(data.error);
				}
...
Vvveb.Gui = {
	...
	saveAjax : function () {
		...
		return Vvveb.Builder.saveAjax(url, null, function (data) {
			if (data.success) {
				$("#message-modal").modal().find(".modal-body").html("Page content have been saved");
			}
		});
	},
		...
		function drawComponentsTree(tree)
		{
			...
			for (i in tree)
			{
				...
				if (tree[i].children.length > 0) 
				{
					...
					<label for="id' + j + '" style="background-image:url(../../assets/pagebuilder/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
